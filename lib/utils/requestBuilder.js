...
function rtiRequestBuilder(req, eventType, params) {
	...
	reqParams[rtiParams.CLIENT_IP] = "77.125.246.171"
...
function getCheqCookie(cookie) {
	return !cookie ? undefined : cookie.split(';').map(c => c.trim()).find(c => c.includes(rtiParams.CHEQ_COOKIE_NAME)).split('=').slice(1).join('=');
}
