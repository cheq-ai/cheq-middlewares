...
function handleRTIResponse(data, res, next, prams, callback) {
    ...
    else if(rtiActions.blockRedirects.includes(data.threatTypeCode) && data.isInvalid && prams.mode === rtiMode.BLOCKING) {
    ...
    else if(rtiActions.captcha.includes(data.threatTypeCode) && data.isInvalid && prams.mode === rtiMode.BLOCKING) {
        callback(data);
    }
    else {
        next();
