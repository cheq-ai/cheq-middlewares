...
function handler(eventType, params) {
	return async function (req, res, next) {
		return new Request(`${config.baseApi}/${config.defaultApiVersion}/${config.rtiPath}`, 'POST')
			.body(rtiRequestBuilder(req, eventType, params), 'form')
			.timeout(params.timeout || config.rtiTimeout)
			.send()
			.then(rtiRes => handleRTIResponse(rtiRes.data, req, res, next, params))
			.catch(e => {
				// TODO send errors
				next();
			});
