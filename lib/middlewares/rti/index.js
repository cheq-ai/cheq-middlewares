...
function rti(options) {
	console.log('[RTI Middleware] Invoked: rti(options)');
	...
	return function(eventType) {
		console.log('[RTI Middleware] Invoked: rti(eventType handler)');
...
function handler(eventType, options) {
	console.log('[RTI Middleware] Invoked: handler(eventType, options)');
	return async function (req, res, next) {
		console.log('[RTI Middleware] Invoked: handler middleware (req, res, next)');
		if (shouldSkip(req, options.URIExclusion)) {
			next();
			return;
		}

		const rtiReq = rtiRequestBuilder(req, eventType, options);
		...
		return request({
			...
			.then(rtiRes => {
				handleRTIResponse(req, res, next, options, rtiReq, rtiRes);
			})
			.catch(error => {
...
function handlerV4(eventType, options) {
	console.log('[RTI Middleware] Invoked: handlerV4(eventType, options)');
	return async function (req, res, next) {
		console.log('[RTI Middleware] Invoked: handlerV4 middleware (req, res, next)');
		if (shouldSkip(req, options.URIExclusion)) {
			next();
		...
		const rtiReqHeaders = req.headers;
		const rtiReq = rtiRequestBuilderV4(req, eventType, options);

		console.log(rtiReqHeaders);
		...
		return request({
			...
			.then(rtiRes => {
				handleRTIResponse(req, res, next, options, rtiReqHeaders, rtiReq, rtiRes);
			})
			.catch(error => {
				logger.warn(JSON.stringify({error, expMiddlewareVersion: rtiConfig.middlewareVersion}), eventType);
				next();
...
function handleRTIResponse(req, res, next, options, rtiReqHeaders, rtiReq, rtiRes) {
	console.log('[RTI Middleware] Invoked: handleRTIResponse');
	res.locals.rtiReqHeaders = rtiReqHeaders;
	res.locals.rtiReq = rtiReq;
...
function shouldSkip(req, URIExclusion) {
	console.log('[RTI Middleware] Invoked: shouldSkip');
