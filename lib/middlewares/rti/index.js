...
function rti(options) {
        ...
        if(!options.hasOwnProperty('tagHash')) {
...
function handleRTIResponse(data, res, next, prams, callback) {
    ...
    else if(rtiActions.blockRedirects.includes(data.threatTypeCode)) {
    ...
    else if(rtiActions.captcha.includes(data.threatTypeCode)) {
        callback(data);
    }
    else {
        next();
