...
function handler(eventType, options) {
	...
	return async function (req, res, next) {
		...
		const slpReqHeaders = req.headers;
		...
		return request({
			...
			.then(slpRes => {
				handleSLPResponse(req, res, next, slpReqHeaders, slpReq, slpRes); 
			})
			.catch(error => {
...
function handleSLPResponse(req, res, next, slpReqHeaders, slpReq, slpRes) {
	console.log('[SLP Middleware] Invoked: handleSLPResponse');
	if (!slpRes) {
		next();
	} else {
		res.locals.slpReqHeaders = slpReqHeaders;
